...
Player.prototype.download = function (src, cached, callback) {
  ...
  urllib.get(src, function (res) {
    ...
    res.on('end', function () {
      debug('downloaded %s', src);
      self.emit('downloaded', src);
    });

    // finish
    pool.on('finish', function () {
      debug('write %s into pool finished', src);
    });

    // 创建只写流
    var writable = fs.createWriteStream(cached);
    pool.pipe(writable);
    debug('writing %s', cached);

    // 错误事件
    writable.on('error', function (err) {
      debug('writable %s error.');
      self.emit('error', err);
    });
    // finish
    writable.on('finish', function () {
      debug('write %s file finished', cached);
    });
  ...
  if (src.indexOf('http') === -1) {
    fs.exists(src, function (exists) {
      if (exists) {
        ...
        callback(new Error(src + ' is not exists.'));
      }
    });
    return;
  }
